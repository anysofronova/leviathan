...
export const Product = ({ name, price, img }: { id?: number; name: string; price: number; img: string }) => {
  return (
    <Link href={`/product/${name}`} className='relative w-full cursor-pointer overflow-hidden p-4 lg:max-w-[300px]'>
      ...
      <img src={img} alt='img' className='w-full transition-all duration-500 hover:scale-110' />
    </Link>
  )
}
